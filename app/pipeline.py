#!/usr/bin/env python3
"""
Batch PII pipeline (**controller / orchestrator**): extract → chunk → multi-detector NER → agreement →
anonymisation → reports.

Domain types live in ``app.models``; NER backends under ``app.services.ner``. Web UI/API: ``app`` (Flask).
"""
...
# NER backends (see ``app.services.ner``)
...
try:
    from app.services.ner.qwen_ollama import detect_pii_with_qwen_ollama as _qwen_ner_detect
    from app.services.ner.qwen_ollama import detect_language as _detect_language
    from app.services.ner.qwen_ollama import judge_disputed_pii_spans as _qwen_judge_spans
...
COMMON_NON_PII_WORDS: frozenset[str] = frozenset({
    ...
    # Long words often hit ALPHANUMERIC_PATTERN (12+ chars) in regulatory / IT prose
    "classification", "implementation", "specification", "understanding", "illustrative",
    "provisioning", "clarifications", "requirements", "infrastructure", "organization",
    "organisations", "organizations", "documentation", "functionality", "applicability",
    "identification", "authentication", "authorization", "confidentiality", "availability",
    "reconciliation", "recommendations", "considerations", "circumstances", "subsequently",
})
...
def setup_logging() -> None:
    ...
    qwen_ner_logger = logging.getLogger("app.services.ner.qwen_ollama")
    qwen_ner_logger.setLevel(logging.DEBUG)
    qwen_ner_logger.addHandler(fh)
    logger.info("Logging to %s", log_file)


def filter_pii_detections_by_min_score(
    detections: list[PiiDetection],
    min_score: float,
) -> list[PiiDetection]:
    if min_score <= 0:
        return detections
    return [p for p in detections if p.score >= min_score]


def filter_finding_dicts_by_min_score(
    findings: list[dict[str, Any]],
    min_score: float,
) -> list[dict[str, Any]]:
    """Serialized findings / per-detector lists: drop rows with score < min_score."""
    if min_score <= 0:
        return findings
    ...
    for f in findings:
        if not isinstance(f, dict):
            continue
        sc = float(f.get("score", 0.8))
        if sc >= min_score:
            out.append(f)
    return out


def filter_chunk_report_scores_inplace(chunk: dict[str, Any], min_score: float) -> None:
    """Remove low-score rows from every list of {value, pii_type, score?} in a chunk report."""
    if min_score <= 0:
        return
    for key, val in list(chunk.items()):
        if not isinstance(val, list) or not val:
            continue
        if not all(isinstance(x, dict) and "value" in x for x in val):
            continue
        chunk[key] = filter_finding_dicts_by_min_score(val, min_score)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.6,
        help="Qwen NER minimum score for returned spans (default: 0.6; align with --min-ner-confidence).",
    )
    parser.add_argument(
        "--min-ner-confidence",
        type=float,
        default=DEFAULT_MIN_NER_CONFIDENCE,
        help="Minimum score for final combined PII list after agreement and precision filter (default: 0.6). "
        "Raw per-detector lists are not clipped before agreement (so GLiNER can contribute despite low scores).",
    )
    parser.add_argument(
        "--min-ner-agreement",
        type=int,
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Do not call Qwen (Ollama) to verify spans dropped by the agreement step; those stay dropped.",
    )
    parser.add_argument(
        "--qwen-python",
        type=str,
        default=sys.executable,
        help="Python executable used to run the anonymize worker script (--qwen-script).",
    )
    parser.add_argument(
        "--qwen-script",
        type=Path,
        default=Path(__file__).resolve().parent / "anonymize" / "ollama.py",
        help="Script for anonymized values (default: app/anonymize/ollama.py via Ollama). "
        "Use app/anonymize/huggingface.py for HF Qwen2.5-1.5B, app/anonymize/stub.py for stub.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    parser.add_argument(
        ...
        choices=["cpu", "gpu"],
        default=None,
        help="Qwen via Ollama: cpu = CPU-only (OLLAMA_NUM_GPU=0), Qwen 3.5 4B defaults; gpu = use GPU if available. Default: config YAML ``mode`` or gpu.",
    )
...
def _relabel_suspicious_from_ner(
    combined_pii: list[PiiDetection],
    ner_groups: list[tuple[str, list[PiiDetection]]],
) -> None:
    """When a finding is labeled suspicious_token but GLiNER or Qwen (or Presidio) predicted a specific type (e.g. aadhaar), use that label instead."""
    text_to_ner_label: dict[str, str] = {}
    for _name, pii_list in ner_groups:
        for p in pii_list:
            ...
            if label == "suspicious_token":
                continue
            # Normalise to canonical: aadhaar when value matches Aadhaar pattern
            if AADHAAR_PATTERN.search(p.text.strip()) or "aadhaar" in label:
                label = "aadhaar number"
            elif PAN_PATTERN.search(p.text.strip()) or ("pan" in label and "number" in label):
                label = "pan number"
            elif "pan" in label:
                label = "pan number"
            existing = text_to_ner_label.get(t)
            if existing is None or label == "aadhaar number" or label == "pan number":
                text_to_ner_label[t] = label
    for p in combined_pii:
        if p.label.lower().strip() != "suspicious_token":
            continue
        t = p.text.strip().lower()
        if t in text_to_ner_label:
            p.label = text_to_ner_label[t]
...
def regex_entropy_audit(text: str) -> list[str]:
    ...
    for pattern in patterns:
        for match in pattern.finditer(text):
            span = match.group(0)
            # ALPHANUMERIC_PATTERN is meant for opaque IDs (often mixed alnum / long). Plain
            # dictionary words of 12–20 letters also match and flood audit as suspicious_token.
            if pattern is ALPHANUMERIC_PATTERN and re.fullmatch(r"[A-Za-z]+", span) and len(span) <= 22:
                continue
            findings.add(span)
...
# Temp dirs with patched gliner_config.json (symlinks to weights); kept for process lifetime
_gliner_patched_dirs: list[Path] = []


def _gliner_resolved_model_dir(model_id: str) -> Path:
    """Local directory containing gliner_config.json (HF cache path or user path)."""
    p = Path(model_id)
...
def _gliner_dir_with_words_splitter_override(model_dir: Path, words_splitter_type: str) -> Path:
    """
    Build a directory that mirrors ``model_dir`` but sets ``words_splitter_type`` in
    ``gliner_config.json``. Other files/dirs are symlinks so we do not duplicate weights.

    Passing ``words_splitter`` into ``GLiNER.from_pretrained`` fails: that kwarg is forwarded to
    the inner PyTorch module. Patching the config file is the supported effect.
    """
    patched = Path(tempfile.mkdtemp(prefix="armor_gliner_"))
    _gliner_patched_dirs.append(patched)
    cfg_name = "gliner_config.json"
    for src in model_dir.iterdir():
        if src.name.startswith("."):
            continue
        dest = patched / src.name
        if src.name == cfg_name:
            data = json.loads(src.read_text(encoding="utf-8"))
            data["words_splitter_type"] = words_splitter_type
...
def _get_gliner_model(model_id: str) -> GLiNER:
    """
    Load GLiNER from Hugging Face. Many checkpoints set ``words_splitter_type: stanza``,
    which requires ``pip install stanza langdetect``. We default to **whitespace** by patching
    a copy of ``gliner_config.json`` (see ``_gliner_dir_with_words_splitter_override``).

    Set ``ARMOR_GLINER_USE_MODEL_SPLITTER=1`` to use the checkpoint's config as-is.
    Set ``ARMOR_GLINER_WORDS_SPLITTER`` to override the patched type (e.g. ``spacy``).
    """
    if model_id not in _gliner_model_cache:
        logger.info("Loading GLiNER model: %s", model_id)
        use_model_default = os.environ.get("ARMOR_GLINER_USE_MODEL_SPLITTER", "").strip().lower() in (
            "1",
            "true",
            "yes",
        )
        if use_model_default:
            _gliner_model_cache[model_id] = GLiNER.from_pretrained(model_id)
        else:
            ws_kind = (os.environ.get("ARMOR_GLINER_WORDS_SPLITTER") or "whitespace").strip() or "whitespace"
            resolved = _gliner_resolved_model_dir(model_id)
            load_from = _gliner_dir_with_words_splitter_override(resolved, ws_kind)
            _gliner_model_cache[model_id] = GLiNER.from_pretrained(str(load_from))
...
def build_qwen_replacement_prompt(
    ...
    user = (
        f"PII detected (Presidio + GLiNER + {qwen_display_name}):\n"
...
def _chunk_report(
    ...
    # Backward compat: UI/run_meta expect key "qwen" for counts
    if "qwen_cpu" in report:
        report["qwen"] = report["qwen_cpu"]
...
def process_chunk(
    ...
    qwen_ner_group_name: str = "qwen",
    qwen_display_name: str = "Qwen",
    min_ner_confidence: float = DEFAULT_MIN_NER_CONFIDENCE,
    qwen_judge_low_agreement: bool = True,
) -> tuple[str, bool, dict[str, Any]]:
    chunk_len = len(chunk)
    min_c = min_ner_confidence
    ...
    if use_qwen_ner and _qwen_ner_detect is not None:
        _report_progress(
            ...
            qwen_label=qwen_display_name,
        )
        logger.info(
            "Requesting %s NER (Ollama %s) (chunk length=%s chars)",
            qwen_display_name,
            qwen_ner_model_name or "?",
            chunk_len,
        )
        try:
            ...
            logger.warning("%s NER (Ollama) failed for chunk: %s", qwen_display_name, exc)
    qwen_pii = _normalize_pii_types_by_pattern(qwen_pii)
    ner_groups.append((qwen_ner_group_name, qwen_pii))
    _report_progress(
        progress_file,
        stage="qwen_ner",
        file=file_name,
    ...
    judge_approved: list[PiiDetection] = []
    if (
        dropped_findings
        and use_qwen_ner
        and qwen_judge_low_agreement
        and _qwen_judge_spans is not None
    ):
        _report_progress(
            progress_file,
            stage="qwen_judge",
            ...
            total_chunks=total_chunks,
            qwen_label=qwen_display_name,
            judge_candidates=len(dropped_findings),
        )
        try:
            approved_pairs = _qwen_judge_spans(chunk, dropped_findings)
        except Exception as exc:  # pragma: no cover
            logger.warning("Qwen judge (low-agreement spans) failed: %s", exc)
            approved_pairs = []
        for value, ptype in approved_pairs:
            base_score = 0.72
            for d in dropped_findings:
                if str(d.get("value", "")).strip() == value and str(d.get("pii_type", "")).strip().lower() == ptype.lower():
                    base_score = max(base_score, min(float(d.get("score", 0.5)), 0.95))
                    break
            judge_approved.append(PiiDetection(text=value, label=ptype, score=base_score))
        if judge_approved:
            ner_groups.append(("qwen_judge", judge_approved))
            combined_pii.extend(judge_approved)
            combined_pii = _dedup_detections(combined_pii)
            logger.info(
                "Qwen judge recovered %s span(s): %s",
                len(judge_approved),
                json.dumps([{"value": p.text, "pii_type": p.label} for p in judge_approved], ensure_ascii=True),
            )
    ...
    # Relabel suspicious_token to aadhaar/pan etc. when GLiNER or Qwen predicted that type for the same value
    _relabel_suspicious_from_ner(combined_pii, ner_groups)
    combined_pii = _dedup_detections(combined_pii)
    combined_pii = _precision_filter(combined_pii)
    combined_pii = filter_pii_detections_by_min_score(combined_pii, min_c)
    ...
    r_system, r_user = build_qwen_replacement_prompt(
        chunk, combined_pii, lang=lang, qwen_display_name=qwen_display_name
    )
...
def process_file(
    ...
    if chunks:
        ...
        anonymized_chunk, success, report = process_chunk(
            ...
            qwen_ner_group_name=getattr(args, "qwen_ner_group_name", "qwen"),
            qwen_display_name=getattr(args, "qwen_display_name", "Qwen"),
            min_ner_confidence=getattr(args, "min_ner_confidence", DEFAULT_MIN_NER_CONFIDENCE),
            qwen_judge_low_agreement=not getattr(args, "no_qwen_judge_low_agreement", False),
        )
...
def main() -> None:
    ...
    mode_in = args.mode if args.mode is not None else cfg.get("mode", "gpu")
    args.mode = apply_qwen_runtime_settings(mode_in, cfg)
    args.qwen_ner_group_name = qwen_ner_source_key(args.mode)
    args.qwen_display_name = qwen_public_display_name(args.mode)
    setup_logging()
    ...
    logger.info(
        "%s (Ollama, mode=%s): %s",
        args.qwen_display_name,
        args.mode,
        "enabled" if (args.use_qwen_ner and _qwen_ner_detect is not None) else "disabled or unavailable",
    )
    if args.mode == "cpu":
        logger.info(
            "Qwen CPU: Ollama CPU-only (OLLAMA_NUM_GPU=%s), NER model=%s — use `ollama pull qwen3.5:4b` if missing.",
            os.environ.get("OLLAMA_NUM_GPU", "0"),
            os.environ.get("OLLAMA_NER_MODEL", "?"),
        )
    ...
    try:
        from app.services.ner.qwen_ollama import get_ollama_ner_model
        _qwen_ner_model = get_ollama_ner_model()
    except Exception:
        _qwen_ner_model = os.environ.get("OLLAMA_NER_MODEL", "?")
    args.qwen_ner_model = getattr(args, "qwen_ner_model", _qwen_ner_model)
    logger.info(
        "NER: Presidio (EN only), GLiNER (xlarge/gretelai/urchade/arabic by language), %s NER (Ollama %s). Language detected per chunk via %s.",
        args.qwen_display_name,
        args.qwen_ner_model,
        args.qwen_display_name,
    )
    logger.info(
        "NER minimum confidence: %s (applied to final combined list after agreement; per-detector scores kept for agreement).",
        args.min_ner_confidence,
    )
    ...
    if not args.no_report and file_reports:
        ...
        try:
            write_armor_state(latest_report=str(rel_report), updated_at=created_at)
            logger.info("State updated: latest_report=%s", rel_report)
        except Exception as exc:
            logger.warning("Could not write instance/state.yaml: %s", exc)
        run_id = timestamp
