#!/usr/bin/env python3
"""Manual test: Presidio + spaCy in isolation (debug empty analyze results). Run: python tests/test_presidio.py"""
...
def main() -> None:
    ...
    try:
        ...
        for i, r in enumerate(results):
            print(
                f"   [{i}] entity_type={getattr(r, 'entity_type', None)} "
                f"start={getattr(r, 'start', None)} end={getattr(r, 'end', None)} "
                f"score={getattr(r, 'score', None)} "
                f"value={TEST_TEXT[getattr(r, 'start', 0):getattr(r, 'end', 0)]!r}"
            )
    except Exception as e:
        print(f"   FAIL: {e}")
        return
